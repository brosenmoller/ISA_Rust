...
use super::{components::*, PLAYER_SPEED, PLAYER_SIZE};
    ...
    commands.spawn(
        (
            SpriteBundle{
                ...
                texture: asset_server.load("sprites/Car_5.png"),
                ..default()
            },
            Player {},
        )
...
pub fn confine_player_movement(
    ...
    if let Ok(mut player_transform) = player_query.get_single_mut() {
        ...
        let half_player_size = (PLAYER_SIZE * SIZE_MULTIPLIER) / 2.0;
...
pub fn player_hit_enemy(
    ...
    if let Ok((player_entity, player_transform)) = player_query.get_single_mut() {
        ...
        for enemy_transform in enemy_query.iter(){
            ...
            let player_radius = (PLAYER_SIZE * SIZE_MULTIPLIER) / 2.0;
            let enemy_radius = (ENEMY_SIZE * SIZE_MULTIPLIER) / 2.0;
...
pub fn player_hit_star(
    ...
    if let Ok(player_transform) = player_query.get_single() {
        ...
        for (star_entity, star_transform) in star_query.iter() {
            ...
            let player_radius = (PLAYER_SIZE * SIZE_MULTIPLIER) / 2.0;
            let star_radius = (STAR_SIZE * SIZE_MULTIPLIER) / 2.0;
